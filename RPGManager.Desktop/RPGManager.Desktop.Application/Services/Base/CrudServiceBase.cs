...
public abstract class CrudServiceBase<TEntity, TRegister, TQuery> : ServiceBase, ICrudServiceBase<TEntity, TRegister, TQuery>
    ...
    where TQuery : class
{
    ...
    public CrudServiceBase( IRepositoryBase<TEntity> repository, IMapper mapper ) : base(mapper)
    {
    ...
    public virtual async Task<Guid> Create( TRegister model )
    {
        var entity = Mapper.ToEntity<TEntity, TRegister>( model );
    ...
    public async Task<IEnumerable<TQuery>> Recover()
    {
        ...
        foreach ( var entity in entities )
            models.Add( Mapper.ToModel<TEntity, TQuery>( entity ) );
    ...
    public async Task Update( Guid id, TRegister model )
    {
        ...
        entity = Mapper.ToEntity<TEntity, TRegister>( model );
        entity.Id = id;
